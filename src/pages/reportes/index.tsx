...
const Index = () => {
  ...
  const [cargando, setCargando] = useState(true); // Evita parpadeos o llamadas API sin auth
  ...
  useEffect(() => {
    ...
    if (adminGuardado && token) {
      ...
      setCargando(false); // Ya tenemos los datos, permitimos mostrar la página
    } else {
      // Redirige inmediatamente si no hay sesión activa
      window.location.href = "http://10.0.40";
    }
  }, []); // Quita 'navigate' de aquí si no lo usas dentro, un arreglo vacío asegura que solo corra al montar el componente

  // Si no está autenticado, bloqueamos el renderizado del HTML de reportes
  if (cargando && !localStorage.getItem("token")) {
  ...
  return (
    ...
    >
      ...
      <div className="container d-flex justify-content-center align-items-center vh-91">
        <div
          className="card report-card p-4 w-100"
          style={{
            maxWidth: "900px",
            borderRadius: "18px",
            minHeight: "500px",
        ...
        >
          {/* Título */}
          <h3 className="text-center mb-4 fw-bold mt-4">PANEL DE REPORTES</h3>
            ...
            <div className="col-md-4 mb-3">
              <div className="p-3 border rounded h-100 hover-card">
                ...
                <button
                  ...
                  onClick={() => navigate("/reportes/administrativos")}
                >
                  Ver reportes
                </button>
              </div>
